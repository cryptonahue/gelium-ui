...
| Version | `0.6.4` | `lib/package.json` and `lib.AssetsVersion`; docs chrome badge; asset query `?v={{.AssetsVersion}}` |
...
- **Entity block** (single source, repeated consistently): name `Gelium UI`, softwareVersion `0.6.4` (`lib.AssetsVersion`), license `MIT`, author `Gelium UI maintainer`.
...
- **Requirements**:
  - a visible provenance line in the article (e.g. "Gelium UI documentation — v0.6.4, MIT. Source: button.md");
...
- **Gelium UI today**: visible version is `lib.AssetsVersion` (`0.6.4`) in the docs chrome and JSON-LD `softwareVersion`; assets use `?v={{.AssetsVersion}}`.
- **Requirements**:
...
| `SoftwareApplication` | each `/components/*` | `name`, `applicationCategory` DeveloperApplication, `softwareVersion` 0.6.4, `operatingSystem`, `license` MIT |
