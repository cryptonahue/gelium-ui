...
Divider is a thin line that groups or separates related content. Use a divider when adjacent content needs a visual separation that is not a container boundary. Use it between list rows, toolbar groups, or stacked sections. It is a native `hr` element styled with the `ui-divider` class, so it needs no component JavaScript at all.

## Guidance

### When to use

Use a divider when adjacent content needs a visual separation that is not a container boundary. Use it between list rows, toolbar groups, or stacked sections.
...
The divider is `display: block`, with no border or margin, clipped to its content box so shortened variants do not fade the paint.
