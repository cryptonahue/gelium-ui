...
func TestMenuPrimitiveCSSMapsMaterialAnatomy(t *testing.T) {
	...
	for _, contract := range []string{
		...
		`--ui-menu-item-height: var(--ui-size-item-sm);`,
		`--ui-menu-item-leading-space: var(--ui-space-3);`,
		`--ui-menu-item-trailing-space: var(--ui-space-3);`,
		`--ui-menu-item-icon-size: var(--ui-size-icon);`,
		`.ui-menu-item {`,
...
func TestMenuDemoClassVocabularyIsClosed(t *testing.T) {
	tmpl := repositoryFile(t, "site", "web", "templates", "menu.html")
	css := regexp.MustCompile(`\s+`).ReplaceAllString(sourceComponentCSS(t, "menu.css"), " ")
