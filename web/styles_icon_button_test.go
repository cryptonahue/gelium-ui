...
func TestIconButtonKeepsFixedMaterialCircleDimensions(t *testing.T) {
	css := regexp.MustCompile(`\s+`).ReplaceAllString(sourceIconButtonCSS(t), " ")
	for _, contract := range []string{
		`.ui-icon-button { position: relative; display: inline-flex; width: 2.5rem; height: 2.5rem; align-items: center; justify-content: center; border: 1px solid transparent; border-radius: var(--ui-radius-full); padding: 0; flex: none; cursor: pointer; text-decoration: none;`,
		`.ui-icon-button .ui-icon { width: 1.5rem; height: 1.5rem;`,
	} {
		if !strings.Contains(css, contract) {
