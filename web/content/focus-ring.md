...
Focus ring is the Loom-wide keyboard focus contract. Every focusable element keeps a visible indicator thanks to a broad `:focus-visible` rule, while `.ui-focus-ring` is the explicit utility for custom focusable elements that are not native controls.
