...
// inlineAlertView is the production view model for the shared "inline-alert"
// partial. Tone is a closed vocabulary (error|warning|success|info); Icon may
// carry a trusted decorative glyph (template.HTML, aria-hidden in the partial).
...
func (s *server) recipeAdminResourceNotFound(w http.ResponseWriter, title, body string) {
	s.renderErrorPage(w, http.StatusNotFound, title, body, true, "/recipes/admin-resource", "Back to projects")
}
